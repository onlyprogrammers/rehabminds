...
export default function SemesternotesPage({ params }: { params: { program: string; semester: string } }) {
  const searchParams = useSearchParams()
  const selectedSubject = searchParams.get("subject")
  const callparam = use(params)
  // Extract program and semester info
  const programName = callparam.program.replace("-notes", "").toUpperCase()
  const semesterName = callparam.semester.replace("-notes", "").replace("-", " ")

  // Mock notes data with multiple notes per subject
  const allnotes = [
    // BCS-011 Multiple notes
    {
      id: "bcs011-note-1",
      code: "BCS-011",
      title: "Computer Basics - note 1 (Detailed Solutions)",
      program: "BCA",
      semester: "1st",
      price: 0,
      originalPrice: 199,
      ...
      reviews: 1250,
      downloads: 5200,
      description: "Comprehensive note covering computer fundamentals with detailed explanations",
    ...
    },
    {
      id: "bcs011-note-2",
      code: "BCS-011",
      title: "Computer Basics - note 2 (Premium Quality)",
      program: "BCA",
      semester: "1st",
      price: 149,
      originalPrice: 299,
      ...
      reviews: 890,
      downloads: 3100,
      description: "Premium quality note with practical examples and screenshots",
      image: "/placeholder.svg?height=200&width=300",
      category: "note" as const,
    ...
    },
    {
      id: "bcs011-note-3",
      code: "BCS-011",
      title: "Computer Basics - note 3 (Expert Written)",
      program: "BCA",
      semester: "1st",
      price: 199,
      originalPrice: 399,
      ...
      reviews: 567,
      downloads: 2200,
      description: "Expert written note with industry insights and real-world examples",
      image: "/placeholder.svg?height=200&width=300",
      category: "note" as const,
      author: "Mr. Vikash Singh",
      pages: 48,
    },
    // BCS-012 Multiple notes
    {
      id: "bcs012-note-1",
      code: "BCS-012",
      title: "Mathematics - note 1 (Step-by-Step Solutions)",
      program: "BCA",
      semester: "1st",
      price: 99,
      originalPrice: 199,
    ...
    },
    {
      id: "bcs012-note-2",
      code: "BCS-012",
      title: "Mathematics - note 2 (Complete Guide)",
      program: "BCA",
      semester: "1st",
      price: 179,
      originalPrice: 299,
    ...
    // ECO-01 Multiple notes
    {
      id: "eco01-note-1",
      code: "ECO-01",
      title: "Business Organization - note 1 (Free Version)",
      program: "BCA",
      semester: "1st",
      price: 0,
      type: "free" as const,
    ...
    },
    {
      id: "eco01-note-2",
      code: "ECO-01",
      title: "Business Organization - note 2 (Premium Edition)",
      program: "BCA",
      semester: "1st",
      price: 129,
      originalPrice: 249,
    ...
    // ECO-02 Multiple notes
    {
      id: "eco02-note-1",
      code: "ECO-02",
      title: "Accountancy-1 - note 1 (Basic Version)",
      program: "BCA",
      semester: "1st",
      price: 99,
      originalPrice: 199,
    ...
    },
    {
      id: "eco02-note-2",
      code: "ECO-02",
      title: "Accountancy-1 - note 2 (Advanced Solutions)",
      program: "BCA",
      semester: "1st",
      price: 199,
      originalPrice: 399,
    ...
    // ENG-01 Multiple notes
    {
      id: "eng01-note-1",
      code: "ENG-01",
      title: "English - note 1 (Grammar Focus)",
      program: "BCA",
      semester: "1st",
      price: 79,
      originalPrice: 149,
    ...
    },
    {
      id: "eng01-note-2",
      code: "ENG-01",
      title: "English - note 2 (Communication Skills)",
      program: "BCA",
      semester: "1st",
      price: 119,
      originalPrice: 199,
  ...
  // Filter notes by selected subject if any
  const filterednotes = selectedSubject
    ? allnotes.filter((note) => note.code === selectedSubject)
    : allnotes

  // Get unique subject codes for filter dropdown
  const uniqueSubjects = [...new Set(allnotes.map((note) => note.code))]
      ...
      <div className="bg-white border-b shadow-sm">
        <div className="container mx-auto px-4 py-4 lg:py-6">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-xl lg:text-3xl font-bold text-gray-900 capitalize">
                {programName} - {semesterName} notes
              </h1>
              {selectedSubject && (
                <p className="text-sm lg:text-base text-blue-600 font-medium">
                  Showing {filterednotes.length} notes for: {selectedSubject}
                </p>
              )}
            </div>
          </div>
          ...
          <p className="text-sm lg:text-lg text-gray-600">
            {selectedSubject
              ? `Multiple note solutions available for ${selectedSubject} subject`
              : `All notes for ${semesterName} semester`}
          </p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white">
        <div className="container mx-auto px-4 py-3">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 text-center mobflex">
            <div>
              <div className="text-2xl lg:text-3xl font-bold mb-1">{filterednotes.length}</div>
              <div className="text-sm text-blue-100">notes</div>
            </div>
            <div>
              <div className="text-2xl lg:text-3xl font-bold mb-1">
                {filterednotes.filter((a) => a.type === "free").length}
              </div>
            ...
            <div>
              <div className="text-2xl lg:text-3xl font-bold mb-1">
                {Math.round(filterednotes.reduce((sum, a) => sum + a.downloads, 0) / 1000)}K+
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {filterednotes.length} different note solutions available
                  </p>
                </div>
              </div>
              <Link href={`/notes/${params.program}/${params.semester}`}>
        ...
        {filterednotes.length > 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            {filterednotes.map((note) => (
              <Link
                key={note.id}
                href={`/notes/${callparam.program}/${callparam.semester}/${note.code.toLowerCase()}/${note.id}`}
              >
                <ProductCard {...note} />
              </Link>
            ))}
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-6">
              {selectedSubject
                ? `No notes available for ${selectedSubject} yet.`
                : `No notes available for ${semesterName} yet.`}
            </p>
            <Link href={`/notes/${params.program}`}>
